...
seed_rdb_volume() { # seed_rdb_volume VOLUME MOUNT KEY VALUE [redis-server extra args...]
  ...
  # SEED_EXTRA_KEYS=N adds N filler keys (pop:1..N) so the seeded RDB can carry
  # a dataset the size of the one the incident lost, not a single marker key.
  if [ "${SEED_EXTRA_KEYS:-0}" -gt 0 ]; then
    docker exec -i seeder sh -c "seq 1 ${SEED_EXTRA_KEYS} | sed 's/.*/SET pop:& x/' \
      | redis-cli -a '$PW' --pipe" >/dev/null 2>&1
  fi
...
# Drop a committed-but-empty appendonlydir into a volume and backdate it, the
# way the bitnami lineage leaves one behind: a service running
# REDIS_AOF_ENABLED=no keeps dump.rdb as its live dataset while an AOF written
# by some older image sits beside it, unwritten, for months. Generated by a
# real redis rather than hand-assembled, so the manifest and its base file are
# genuinely loadable — which is what made this shape dangerous.
# AGE is a bare number of days (default 434, the incident's), any `touch -d`
# expression ("15 minutes ago"), or `live` to leave the files at the present.
# SUBDIR places the AOF under a nested dataset dir (bitnami's `redis/data`).
# KEY VALUE, when given, are written into the AOF before it is committed, so
# the stale AOF carries data of its own rather than an empty base.
inject_stale_committed_aof() { # inject_stale_committed_aof VOLUME MOUNT [AGE] [SUBDIR] [KEY VALUE]
  local vol="$1" mount="$2" age="${3:-434}" subdir="${4:-}" key="${5:-}" value="${6:-}"
  local dir="${mount}${subdir:+/$subdir}" touch_cmd="true"
  case "$age" in
    live) ;;
    ''|*[!0-9]*) touch_cmd="find ${dir}/appendonlydir -exec touch -d '${age}' {} +" ;;
    *) touch_cmd="find ${dir}/appendonlydir -exec touch -d '${age} days ago' {} +" ;;
  esac
  docker run --rm -v "${vol}:${mount}" "$SEED_IMAGE" sh -c "
    set -e
    redis-server --port 7777 --appendonly yes --dir /tmp --daemonize yes
    for i in \$(seq 1 30); do
      redis-cli -p 7777 PING 2>/dev/null | grep -q PONG && break
      sleep 1
    done
    if [ -n '${key}' ]; then redis-cli -p 7777 SET '${key}' '${value}' >/dev/null; sleep 1.5; fi
    redis-cli -p 7777 SHUTDOWN NOSAVE 2>/dev/null || true
    sleep 1
    test -e /tmp/appendonlydir/appendonly.aof.manifest
    mkdir -p ${dir}
    rm -rf ${dir}/appendonlydir
    cp -a /tmp/appendonlydir ${dir}/appendonlydir
    chown -R 999:999 ${dir}/appendonlydir
    ${touch_cmd}
  " >/dev/null 2>&1
}

# Seed a volume the way an AOF-persisting service leaves it after a clean
# stop: `appendonly yes`, one key written, then SHUTDOWN with save — so the
# AOF holds the data and dump.rdb is the shutdown snapshot written moments
# after the AOF fsync. This is the ordering the adoption decision must never
# read as an abandoned AOF.
seed_aof_volume() { # seed_aof_volume VOLUME MOUNT KEY VALUE
  local vol="$1" mount="$2" key="$3" value="$4"
  docker rm -f seeder >/dev/null 2>&1
  docker run --rm -v "${vol}:${mount}" alpine:latest chown 999:999 "$mount" >/dev/null 2>&1
  docker run -d --name seeder --label "$LABEL" -v "${vol}:${mount}" "$SEED_IMAGE" \
    redis-server --requirepass "$PW" --appendonly yes --dir "$mount" >/dev/null
  local i
  for i in $(seq 1 30); do
    docker exec seeder redis-cli -a "$PW" PING 2>/dev/null | grep -q PONG && break
    sleep 1
  ...
  docker exec seeder redis-cli -a "$PW" SET "$key" "$value" >/dev/null 2>&1
  sleep 1.5
  docker exec seeder redis-cli -a "$PW" SHUTDOWN >/dev/null 2>&1
  for i in $(seq 1 30); do
    [ "$(docker inspect -f '{{.State.Running}}' seeder 2>/dev/null)" = "false" ] && break
    sleep 1
  done
  docker rm -f seeder >/dev/null 2>&1
}

# Count the keys in a volume's dump.rdb by loading a COPY of it in a throwaway
# redis-server from the image under test (same RDB format as the node that
# wrote it), with the volume mounted read-only. This reads the file, not the
# running node: a node can answer GET from its AOF while the RDB beside it
# has already been overwritten — which is exactly the state the incident left
# behind. Prints the count, or nothing when the file cannot be loaded.
rdb_keys_in_volume() { # rdb_keys_in_volume VOLUME MOUNT [RDB_RELPATH]
  local vol="$1" mount="$2" rel="${3:-dump.rdb}"
  docker run --rm --entrypoint sh -v "${vol}:${mount}:ro" "$IMAGE" -c "
    set -e
    cp ${mount}/${rel} /tmp/dump.rdb
    redis-server --port 7778 --dir /tmp --dbfilename dump.rdb --appendonly no --save '' \
      --protected-mode no --daemonize yes --logfile /tmp/r.log
    for i in \$(seq 1 60); do
      redis-cli -p 7778 PING 2>/dev/null | grep -q PONG && break
      sleep 0.5
    done
    redis-cli -p 7778 DBSIZE
    redis-cli -p 7778 SHUTDOWN NOSAVE >/dev/null 2>&1 || true
  " 2>/dev/null | tail -1 | tr -cd '0-9'
}

# Run the incident's boot sequence on a prepared volume and prove the live
# RDB survives it end to end. The volume holds a live dump.rdb (EXPECTED_KEYS
# keys, KEY=VALUE among them) beside a committed AOF an older image left
# behind. Asserts, in order: the stale AOF is recognised; the migration
# completes; the node serves KEY; ABSENT_KEY (data only the stale AOF had) is
# not served; the RDB on disk still holds EXPECTED_KEYS; the stale AOF was
# preserved, not deleted; then the destructive half — the stop whose shutdown
# save wrote 0 keys over 57,401 in the incident — leaves EXPECTED_KEYS in the
# RDB and the node serving KEY after it, with the adoption having fired once.
check_live_rdb_wins() { # check_live_rdb_wins T NODE VOL MOUNT RDB_RELPATH EXPECTED_KEYS KEY VALUE [ABSENT_KEY]
  local t="$1" n="$2" vol="$3" mount="$4" rel="$5" expected="$6" key="$7" value="$8" absent="${9:-}"
  local aofdir; aofdir="$(dirname "$rel")"; [ "$aofdir" = "." ] && aofdir=""
  local count
  wait_for_log_line "$n" "appendonlydir predates the RDB on this volume" \
    || { ko "$t" "stale AOF was not recognized; boot trusted the manifest" "$n"; return 1; }
  wait_for_log_line "$n" "enabled AOF after loading adopted RDB" \
    || { ko "$t" "AOF migration never completed" "$n"; return 1; }
  [ "$(rcli "$n" GET "$key")" = "$value" ] \
    || { ko "$t" "live dataset not served — the incident reproduced" "$n"; return 1; }
  if [ -n "$absent" ]; then
    [ -z "$(rcli "$n" GET "$absent")" ] \
      || { ko "$t" "the stale AOF's own data was served: it was loaded, not the RDB" "$n"; return 1; }
  fi
  count=$(rdb_keys_in_volume "$vol" "$mount" "$rel")
  [ "$count" = "$expected" ] \
    || { ko "$t" "dump.rdb holds ${count:-unreadable} keys after the adoption boot, expected ${expected}" "$n"; return 1; }
  docker run --rm -v "${vol}:/v" alpine:latest \
    sh -c "test -e /v/${aofdir:+$aofdir/}appendonlydir.superseded-*/appendonly.aof.manifest" >/dev/null 2>&1 \
    || { ko "$t" "superseded AOF was not preserved" "$n"; return 1; }
  # The destructive half.
  docker stop -t 30 "$n" >/dev/null 2>&1
  count=$(rdb_keys_in_volume "$vol" "$mount" "$rel")
  [ "$count" = "$expected" ] \
    || { ko "$t" "the shutdown save left ${count:-unreadable} keys in dump.rdb, expected ${expected} — this is the write that destroyed the incident volume" "$n"; return 1; }
  docker start "$n" >/dev/null 2>&1
  wait_for_ping "$n" || { ko "$t" "did not come back after restart" "$n"; return 1; }
  [ "$(rcli "$n" GET "$key")" = "$value" ] \
    || { ko "$t" "dataset lost across the restart that saves the RDB" "$n"; return 1; }
  count=$(docker logs "$n" 2>&1 | grep -c "adopted dataset has an RDB")
  [ "$count" = "1" ] || { ko "$t" "adoption fired ${count} times, expected 1" "$n"; return 1; }
  return 0
}
...
# The 2026-09-15 incident, end to end. A bitnami-lineage volume running
# REDIS_AOF_ENABLED=no holds the live dataset in dump.rdb, beside a loadable
# AOF that an older image stopped writing over a year earlier. Keying the
# adoption decision on the manifest's mere existence booted that empty AOF —
# "Done loading RDB, keys loaded: 0" against 57,401 real ones — and the
# shutdown save then wrote the empty dataset over the customer's RDB. The
# newer RDB has to win, and it has to still be there after the restart that
# previously destroyed it.
t_stale_aof_loses_to_newer_rdb() {
  local t=t_stale_aof_loses_to_newer_rdb n=stale-1
  mkvol stale-vol
  seed_rdb_volume stale-vol /data livekey livevalue
  inject_stale_committed_aof stale-vol /data 434
  docker run --rm -v stale-vol:/v alpine:latest \
    test -e /v/appendonlydir/appendonly.aof.manifest >/dev/null 2>&1 \
    || { ko "$t" "seeding produced no committed AOF — test premise broken"; return; }
  start_node "$n" stale-vol /data
  wait_for_log_line "$n" "appendonlydir predates the RDB on this volume" \
    || { ko "$t" "stale AOF was not recognized; boot trusted the manifest" "$n"; return; }
  wait_for_log_line "$n" "enabled AOF after loading adopted RDB" \
    || { ko "$t" "AOF migration never completed" "$n"; return; }
  [ "$(rcli "$n" GET livekey)" = "livevalue" ] \
    || { ko "$t" "RDB dataset lost to the stale AOF — the incident reproduced" "$n"; return; }
  # Quarantined, not deleted: a superseded AOF is still the only record of
  # whatever the older image accepted before it went quiet.
  docker run --rm -v stale-vol:/v alpine:latest \
    sh -c 'test -e /v/appendonlydir.superseded-*/appendonly.aof.manifest' >/dev/null 2>&1 \
    || { ko "$t" "superseded AOF was not preserved" "$n"; return; }
  # The destructive half: this restart is where the shutdown save overwrote
  # dump.rdb with an empty dataset.
  docker restart "$n" >/dev/null 2>&1
  wait_for_ping "$n" || { ko "$t" "did not come back after restart" "$n"; return; }
  [ "$(rcli "$n" GET livekey)" = "livevalue" ] \
    || { ko "$t" "dataset lost across the restart that saves the RDB" "$n"; return; }
  # One adoption only: the committed AOF this boot wrote must now be the
  # source, not re-displaced by the RDB its own shutdown save refreshed.
  local count
  count=$(docker logs "$n" 2>&1 | grep -c "adopted dataset has an RDB")
...
# The incident volume, replayed at its real size: the bitnami layout (dataset
# nested under <mount>/redis/data), 57,401 keys in dump.rdb, and beside them a
# committed AOF with an empty base that an older image wrote 434 days earlier
# and nothing touched since. Every assertion of check_live_rdb_wins runs on
# it, including the key count read from the RDB file itself before and after
# the stop that wrote 0 keys over the customer's data.
t_incident_volume_replays_intact() {
  local t=t_incident_volume_replays_intact n=incident-1 mount=/bitnami/redis/data
  mkvol incident-vol
  SEED_EXTRA_KEYS=57400 seed_rdb_volume incident-vol "$mount" livekey livevalue
  docker run --rm -v incident-vol:/v alpine:latest sh -c \
    'mkdir -p /v/redis/data && mv /v/dump.rdb /v/redis/data/ && chown -R 999:999 /v/redis' \
    >/dev/null 2>&1
  inject_stale_committed_aof incident-vol "$mount" 434 redis/data
  local seeded
  seeded=$(rdb_keys_in_volume incident-vol "$mount" redis/data/dump.rdb)
  [ "$seeded" = "57401" ] \
    || { ko "$t" "seeding produced ${seeded:-unreadable} keys, expected 57401 — test premise broken"; return; }
  start_node "$n" incident-vol "$mount" -e SENTINEL_ENABLED=
  check_live_rdb_wins "$t" "$n" incident-vol "$mount" redis/data/dump.rdb 57401 livekey livevalue || return
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}

# The stale AOF is not empty this time: it holds a key of its own from the era
# it was written. The live RDB must still win, and the stale key must not be
# served — proof that the RDB was loaded, not the AOF, and that the two were
# not merged.
t_stale_aof_with_data_loses_to_newer_rdb() {
  local t=t_stale_aof_with_data_loses_to_newer_rdb n=staledata-1
  mkvol staledata-vol
  seed_rdb_volume staledata-vol /data livekey livevalue
  inject_stale_committed_aof staledata-vol /data 434 "" stalekey stalevalue
  start_node "$n" staledata-vol /data -e SENTINEL_ENABLED=
  check_live_rdb_wins "$t" "$n" staledata-vol /data dump.rdb 1 livekey livevalue stalekey || return
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}

# The margin's edge from above: an AOF that stopped being written 15 minutes
# before the RDB was last saved is past the 10-minute margin and loses. Nothing
# about the decision depends on the gap being years.
t_stale_aof_past_margin_loses_to_newer_rdb() {
  local t=t_stale_aof_past_margin_loses_to_newer_rdb n=margin-1
  mkvol margin-vol
  seed_rdb_volume margin-vol /data livekey livevalue
  inject_stale_committed_aof margin-vol /data "15 minutes ago"
  start_node "$n" margin-vol /data -e SENTINEL_ENABLED=
  check_live_rdb_wins "$t" "$n" margin-vol /data dump.rdb 1 livekey livevalue || return
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}

# The other side of the decision: a service that genuinely persists via AOF,
# with a dump.rdb from days ago beside it (a BGSAVE that predates the AOF's
# recent writes). The AOF is the live dataset and must keep its claim — no
# adoption fires, the AOF's key is served, the old RDB's key is not, and a
# restart changes none of that.
t_live_aof_keeps_claim_over_older_rdb() {
  local t=t_live_aof_keeps_claim_over_older_rdb n=liveaof-1
  mkvol liveaof-vol
  seed_rdb_volume liveaof-vol /data rdbkey rdbvalue
  inject_stale_committed_aof liveaof-vol /data live "" aofkey aofvalue
  docker run --rm -v liveaof-vol:/data "$SEED_IMAGE" \
    sh -c "touch -d '3 days ago' /data/dump.rdb" >/dev/null 2>&1
  start_node "$n" liveaof-vol /data -e SENTINEL_ENABLED=
  wait_for_ping "$n" || { ko "$t" "node did not come up" "$n"; return; }
  [ "$(rcli "$n" GET aofkey)" = "aofvalue" ] \
    || { ko "$t" "the live AOF's data was not served" "$n"; return; }
  [ -z "$(rcli "$n" GET rdbkey)" ] \
    || { ko "$t" "the older RDB was adopted over a live AOF" "$n"; return; }
  local count
  count=$(docker logs "$n" 2>&1 | grep -c "adopted dataset has an RDB")
  [ "$count" = "0" ] || { ko "$t" "adoption fired ${count}x on a live-AOF volume" "$n"; return; }
  docker stop -t 30 "$n" >/dev/null 2>&1
  docker start "$n" >/dev/null 2>&1
  wait_for_ping "$n" || { ko "$t" "did not come back after restart" "$n"; return; }
  [ "$(rcli "$n" GET aofkey)" = "aofvalue" ] \
    || { ko "$t" "live AOF data lost across a restart" "$n"; return; }
  count=$(docker logs "$n" 2>&1 | grep -c "adopted dataset has an RDB")
  [ "$count" = "0" ] || { ko "$t" "adoption fired ${count}x across the restart" "$n"; return; }
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}

# The ordering every AOF service leaves after a clean stop: Redis fsyncs the
# AOF and only then writes the shutdown RDB, so dump.rdb is newer than the AOF
# by moments — never a reason to displace the AOF. Seeded by a real redis with
# `appendonly yes` and a saving SHUTDOWN.
t_shutdown_rdb_moments_newer_than_live_aof_keeps_aof() {
  local t=t_shutdown_rdb_moments_newer_than_live_aof_keeps_aof n=cleanstop-1
  mkvol cleanstop-vol
  seed_aof_volume cleanstop-vol /data aofkey aofvalue
  docker run --rm -v cleanstop-vol:/v alpine:latest \
    sh -c 'test -e /v/dump.rdb && test -e /v/appendonlydir/appendonly.aof.manifest' >/dev/null 2>&1 \
    || { ko "$t" "seeding did not leave both an RDB and a committed AOF — test premise broken"; return; }
  start_node "$n" cleanstop-vol /data -e SENTINEL_ENABLED=
  wait_for_ping "$n" || { ko "$t" "node did not come up" "$n"; return; }
  [ "$(rcli "$n" GET aofkey)" = "aofvalue" ] \
    || { ko "$t" "dataset not served after a clean-stop volume was adopted" "$n"; return; }
  local count
  count=$(docker logs "$n" 2>&1 | grep -c "adopted dataset has an RDB")
  [ "$count" = "0" ] \
    || { ko "$t" "a shutdown RDB moments newer than its AOF was read as abandonment (${count}x)" "$n"; return; }
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}

# The conversion path on the incident volume: the root carries the live RDB
# and the stale AOF, and a replica syncs from it. The replica must receive the
# live dataset, not the empty one the stale AOF would have produced.
t_replication_from_incident_shape() {
  local t=t_replication_from_incident_shape
  mkvol srepl-vol-1; mkvol srepl-vol-2
  seed_rdb_volume srepl-vol-1 /data livekey livevalue
  inject_stale_committed_aof srepl-vol-1 /data 434
  start_node srepl-1 srepl-vol-1 /data \
    -e SENTINEL_HOSTS="srepl-1:26379,srepl-2:26379"
  start_node srepl-2 srepl-vol-2 /data \
    -e SENTINEL_HOSTS="srepl-1:26379,srepl-2:26379" -e REPLICA_OF=srepl-1:6379
  wait_for_log_line srepl-1 "appendonlydir predates the RDB on this volume" \
    || { ko "$t" "root did not recognize the stale AOF" srepl-1; return; }
  wait_for_log_line srepl-1 "enabled AOF after loading adopted RDB" \
    || { ko "$t" "root never finished adoption" srepl-1; return; }
  wait_for_key srepl-2 livekey livevalue \
    || { ko "$t" "replica never received the live dataset" srepl-1 srepl-2; return; }
  [ "$(rcli srepl-2 DBSIZE | tr -cd '0-9')" = "1" ] \
    || { ko "$t" "replica dataset size is not the live RDB's" srepl-1 srepl-2; return; }
  docker rm -f srepl-1 srepl-2 >/dev/null 2>&1
  ok "$t"
}
...
# that finds it unchanged trusts the AOF whatever the gap. Standalone, like
# the auto-patched service: a Sentinel-managed master is fenced by
# min-replicas-to-write until the quorum watcher's first poll, which would
# reject the marker write silently.
t_idle_restart_trusts_own_aof() {
  local t=t_idle_restart_trusts_own_aof n=idle-1
  mkvol idle-vol
  start_node "$n" idle-vol /data -e SENTINEL_ENABLED=
  wait_for_ping "$n" || { ko "$t" "node did not come up" "$n"; return; }
  write_key "$n" idlekey idlevalue || { ko "$t" "node never accepted the write" "$n"; return; }
  ...
  [ "$(rdb_keys_in_volume idle-vol /data)" = "1" ] \
    || { ko "$t" "the shutdown RDB does not hold the written key — test premise broken" "$n"; return; }
...
# timestamp comparison decides again: the newer RDB is adopted, and the data
# in it survives.
t_rdb_saved_by_another_image_is_adopted() {
  ...
  " >/dev/null 2>&1
  docker start "$n" >/dev/null 2>&1
  check_live_rdb_wins "$t" "$n" idle-vol /data dump.rdb 1 idlekey idlevalue || return
  docker rm -f "$n" >/dev/null 2>&1
  ok "$t"
}
...
t_wiped_dataset_with_surviving_conf_does_not_wipe_cluster() {
  ...
  wait_for_log_line dwipe-1 "own sentinel.conf names" 5 \
