...
inject_stale_committed_aof() { # inject_stale_committed_aof VOLUME MOUNT [age_days]
  local vol="$1" mount="$2" days="${3:-434}"
  docker run --rm -v "${vol}:${mount}" "$SEED_IMAGE" sh -c "
    ...
    rm -rf ${mount}/appendonlydir
    cp -a /tmp/appendonlydir ${mount}/appendonlydir
    find ${mount}/appendonlydir -exec touch -d '${days} days ago' {} +
  " >/dev/null 2>&1
}
